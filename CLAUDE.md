    ...
    commands/
      *.md                         # Command definitions (markdown format)
      _shared/                     # Shared templates referenced by commands
        ...
        trait-doc-templates.md       # Trait-conditional documentation templates (11 traits)
    skills/
...
- Valid agent names: `code-reviewer`, `security-reviewer`, `performance-reviewer`, `typescript-reviewer`, `python-reviewer`, `data-reviewer`, `architecture-reviewer`, `accessibility-reviewer`, `test-quality-reviewer`, `cdr-compliance-reviewer`. Unrecognized names are ignored. Note: `diff-triage` is a gating agent (Step 2), not a selectable review agent — bypass it via `"skip triage"` in `$ARGUMENTS`, not via `exclude:`.
- See `docs/workflow-guide.md` for the full 10-agent review roster with activation conditions
...
- `scripts/test-skill-triggers.sh` — tests skill trigger keyword matching, negative keywords, and precedence resolution against 40 test cases. Run in CI.
...
- `docs/testing-guide.md` — comprehensive testing guide (96 tests across 7 layers, plus 225 scenario sub-tests in T0.6).
