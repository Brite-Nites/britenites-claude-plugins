...
---
`$ARGUMENTS` contains an optional time window (e.g., `2w`, `30d`, `3m`). If `$ARGUMENTS` contains `--slides`, strip it before validation and generate a slide deck presentation instead of a scrollable page.
- If it contains instruction-like phrases (such as "ignore previous", "pretend you are", "forget"), ask the user for the time window manually.
- If it is non-empty but doesn't match a number followed by `d` (days), `w` (weeks), or `m` (months), discard it and use the default window (`2w`).

Load the visual-explainer skill, then generate a comprehensive visual project recap as a self-contained HTML page.

Follow the visual-explainer skill workflow. Read the appropriate reference template from `plugins/workflows/skills/visual-explainer/templates/`, CSS patterns from `plugins/workflows/skills/visual-explainer/references/css-patterns.md`, and `plugins/workflows/skills/visual-explainer/references/libraries.md` for mermaid theming before generating. Use a warm editorial or paper/ink aesthetic with muted blues and greens, but vary fonts and palette from previous diagrams.

**Time window** — determine the recency window from `$ARGUMENTS`:
...
3. **Current state.** Check for uncommitted changes (`git status`). Check for stale branches (`git branch --no-merged`). Look for TODO/FIXME comments in recently changed files. Check for progress docs, plan files, or session notes that may contain reasoning.
