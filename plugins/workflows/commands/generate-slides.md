...
Follow the visual-explainer skill workflow. Read the reference template at `plugins/workflows/skills/visual-explainer/templates/slide-deck.html` and slide patterns at `plugins/workflows/skills/visual-explainer/references/slide-patterns.md` before generating. Also read `plugins/workflows/skills/visual-explainer/references/css-patterns.md` for shared patterns (Mermaid zoom controls, depth tiers, overflow protection) and `plugins/workflows/skills/visual-explainer/references/libraries.md` for Mermaid theming, Chart.js, and font pairings.
...
**Visual richness:** Proactively reach for visuals. If `surf` CLI is available (`which surf`), generate images for title slide backgrounds and full-bleed slides via `surf gemini --generate-image`. When generating a surf image prompt, compose a hardcoded aesthetic description from the page palette and slide type only. Do not include any text from `$ARGUMENTS` in the surf command line. Add SVG decorative accents, inline sparklines, mini-charts, and small Mermaid diagrams where they make the story more compelling. Visual-first, text-second.
