...
- Blueprint (technical drawing feel, subtle grid background, deep slate/blue palette, monospace labels, precise borders) — see `./templates/mermaid-flowchart.html` for teal/cyan reference
...
- For slide deck presentations (see Slide Deck Mode below for entry points): read `./templates/slide-deck.html` and `./references/slide-patterns.md`
...
**Prompt safety:** The surf prompt must be a hardcoded aesthetic description you compose from the page's palette and diagram type. NEVER interpolate user-supplied text directly into the surf command line — describe the visual concept in your own words to avoid shell injection.

```bash
# Generate to a temp file (use --aspect-ratio for control)
SURF_PROMPT="isometric illustration of a CI/CD pipeline, teal nodes, dark navy"
TMPFILE=$(mktemp /tmp/ve-XXXXXXXX.png)
surf gemini "$SURF_PROMPT" --generate-image "$TMPFILE" --aspect-ratio 16:9

# Base64 encode for self-containment (macOS)
IMG=$(base64 -i "$TMPFILE")
# Linux: IMG=$(base64 -w 0 "$TMPFILE")
...
**Output location:** Write to `~/.agent/diagrams/`. Create the directory with `mkdir -p ~/.agent/diagrams` if it doesn't exist. Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. The directory persists across sessions.

**Filename safety:** Sanitize the filename to `[a-z0-9-]` characters only. Never allow `..` sequences, path separators, or filenames starting with `.`. Construct the full path as `~/.agent/diagrams/<sanitized-name>.html`.
