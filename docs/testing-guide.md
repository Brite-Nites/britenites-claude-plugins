...
| T6.26 | `cdr-compliance-reviewer` | `/workflows:review` (when project CLAUDE.md has `## Company Context` with `handbook-library`, CLAUDE.md enables, or `comprehensive` depth) | CDR compliance findings with CDR ID references, violation descriptions, compliance verdict. Graceful skip when Context7 unavailable. |
